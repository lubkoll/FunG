...
namespace FunG
{
    ...
    struct Max
    {
        ...
        auto d1( Arg&& dx ) const
        {
            ...
            return ComputeConditional< D1F, D1G >(D1F(f_, dx), D1G(g_, dx), f_bigger_than_g_)();
        }
        ...
        auto d2( ArgX&& dx, ArgY&& dy ) const
        {
            ...
            return ComputeConditional< D2F, D2G >(D2F(f_, dx, dy), D2G(g_, dx, dy), f_bigger_than_g_)();
        }
        ...
        auto d3( ArgX&& dx, ArgY&& dy, ArgZ&& dz ) const
        {
            ...
            return ComputeConditional< D3F, D3G >(D3F(f_, dx, dy, dz), D3G(g_, dx, dy, dz), f_bigger_than_g_)();
        }
    ...
    {
        return Max<std::decay_t<F>, std::decay_t<G>>(std::forward<F>(f), std::forward<G>(g));
    }
    ...
    decltype(auto) max(F&& f, G&& g)
    {
        return Max<std::decay_t<F>, Constant<std::decay_t<G>>>(std::forward<F>(f), constant(std::forward<G>(g)));
    }
    ...
    decltype(auto) max(F&& f, G&& g)
    {
        return Max<Constant<std::decay_t<F>>, std::decay_t<G>>(constant(std::forward<F>(f)), std::forward<G>(g));
    }
    /** @} */
}
