...
{
  auto neoHooke = RFFGen::incompressibleNeoHooke(c0,I);
  auto f = runTest<M>(neoHooke);
...
{
  auto neoHooke = RFFGen::compressibleNeoHooke<Pow<2>,LN>(c0,d0,d1,I);
  auto f = runTest<M>(neoHooke);
  EXPECT_DOUBLE_EQ( std::get<0>(f) , 0 );
...
{
  auto neoHooke = RFFGen::modifiedIncompressibleNeoHooke(c0,I);
  auto f = runTest<M>(neoHooke);
  EXPECT_DOUBLE_EQ( std::get<0>(f) , 0 );
...
{
  auto neoHooke = RFFGen::modifiedCompressibleNeoHooke<Pow<2>,LN>(c0,d0,d1,I);
  auto f = runTest<M>(neoHooke);
  EXPECT_DOUBLE_EQ( std::get<0>(f) , 0 );
