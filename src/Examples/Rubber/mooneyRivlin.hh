...
#ifndef RFFGEN_MOONEY_RIVLIN_HH
#define RFFGEN_MOONEY_RIVLIN_HH
...
namespace RFFGen
{
   ...
   * \brief Generate an "incompressible" Mooney-Rivlin material law \f$ W(F)=c_0\iota_1(F^T F) + c_1\iota_2(F^T F) \f$,
   * where \f$\iota_1\f$ is the first and \f$\iota_2\f$ the second principal matrix invariant.
   * Initialized with the unit matrix.
   */
  template <class Matrix>
  auto incompressibleMooneyRivlin(double c0, double c1)
  {
    return incompressibleMooneyRivlin(c0,c1,LinearAlgebra::unitMatrix<Matrix>());
  }
  ...
  {
    return compressibleMooneyRivlin<InflationPenalty,CompressionPenalty>(c0,c1,d0,d1,LinearAlgebra::unitMatrix<Matrix>());
  }
