...
namespace FunG
{
    namespace MathematicalOperations
    {
         ...
         * @brief %Dot \f$f \cdot g\f$ of functions of type F and G (F and G must satisfy the requirements of Concepts::FunctionConcept).
         */
        template <class F, class G,
                  class = Concepts::FunctionConceptCheck<F> ,
                  class = Concepts::FunctionConceptCheck<G> >
        struct Dot: Chainer<Dot<F,G,Concepts::FunctionConceptCheck<F>,Concepts::FunctionConceptCheck<G> > >
        {
        private:
            template <class IndexedArg>
            using D1Type =
            ComputeSum<
            ComputeDot< D1<F,IndexedArg> , D0<G> >,
            ComputeDot< D0<F> , D1<G,IndexedArg> > >;

            template <class IndexedArgX , class IndexedArgY >
            using D2Type =
            ComputeSum<
            ComputeDot< D2<F,IndexedArgX,IndexedArgY> , D0<G> >,
            ...
            using D3Type =
            ComputeSum<
            ComputeDot< D3<F,IndexedArgX,IndexedArgY,IndexedArgZ> , D0<G> >,
            ...
            using ReturnType = decay_t<decltype( std::declval<F>()().dot(std::declval<G>()()) )>;
        public:
            ...
            {
                return sum( dot_impl( D2<F,IndexedArgX,IndexedArgY>(f,dx,dy) , D0<G>(g) ),
            ...
            {
                return sum( dot_impl( D3<F,IndexedArgX,IndexedArgY,IndexedArgZ>(f,dx,dy,dz) , D0<G>(g) ),
                            dot_impl( D2<F,IndexedArgX,IndexedArgY>(f,dx,dy) , D1<G,IndexedArgZ>(g,dz) ),
                            dot_impl( D2<F,IndexedArgX,IndexedArgZ>(f,dx,dz) , D1<G,IndexedArgY>(g,dy) ),
                            dot_impl( D1<F,IndexedArgX>(f,dx) , D2<G,IndexedArgY,IndexedArgZ>(g,dy,dz) ),
                            dot_impl( D2<F,IndexedArgY,IndexedArgZ>(f,dy,dz) , D1<G,IndexedArgX>(g,dx) ),
                            dot_impl( D1<F,IndexedArgY>(f,dy) , D2<G,IndexedArgX,IndexedArgZ>(g,dx,dz) ),
                            dot_impl( D1<F,IndexedArgZ>(f,dz) , D2<G,IndexedArgX,IndexedArgY>(g,dx,dy) ),
                            dot_impl( D0<F>(f) , D3<G,IndexedArgX,IndexedArgY,IndexedArgZ>(g,dx,dy,dz) ) )();
            }
