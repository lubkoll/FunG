...
namespace FunG
{
    ...
    namespace VariableDetail
    {
        ...
        namespace Has
        {
            ...
            template <class Type> struct Variable : IsVariable<Type> {};

            template <template <class,class> class G, class F>
            struct Variable< G<F,Concepts::FunctionConceptCheck<F> > > : std::integral_constant< bool, Variable<F>::value > {};

            template <template <class,class,class,class> class H, class F, class G>
            struct Variable< H<F,G,Concepts::FunctionConceptCheck<F>,Concepts::FunctionConceptCheck<G> > >
                    : std::integral_constant< bool , Variable<F>::value || Variable<G>::value > {};


            template <class, int id> struct VariableId : std::false_type {};

            template <class Type, int id0, int id>
            struct VariableId< FunG::Variable<Type,id0> , id > : std::integral_constant<bool,id==id0> {};

            template <template <class,class> class G, class F, int id>
            struct VariableId< G<F,Concepts::FunctionConceptCheck<F> > , id >
                    : std::integral_constant< bool, VariableId<F,id>::value > {};

            template <template <class,class,class,class> class H, class F, class G, int id>
            struct VariableId< H<F,G,Concepts::FunctionConceptCheck<F>,Concepts::FunctionConceptCheck<G> > , id >
                    : std::integral_constant< bool , VariableId<F,id>::value || VariableId<G,id>::value > {};
        }
     ...
     *  @{ */
        ...
        namespace Has
        {
            ...
            template <class T>
            constexpr bool variable() { return VariableDetail::Has::Variable<T>::value; }

            /// Check if T contains a type Variable<Type,id>.
            template <class T, int id>
            constexpr bool variableId() { return VariableDetail::Has::VariableId<T,id>::value; }
