...
#ifndef FUNG_PERFORMANCE_TEST_EXAMPLE_TEST_DYNAMIC_ARMADILLO_HH
#define FUNG_PERFORMANCE_TEST_EXAMPLE_TEST_DYNAMIC_ARMADILLO_HH
...
void testExamples_DynamicArmadillo()
{
  ...
  cout << FunG::Checks::AccessTo_n_rows<M>() << endl;
  cout << FunG::Checks::isDynamicMatrix<M>() << endl;
  auto c0 = 1.;
  auto c1 = 1.;
  auto d0 = 1.;
  auto d1 = 1.;
  ...
  fiberTensor.zeros();
  fiberTensor(0,0) = 1.;
  cout << FunG::LinearAlgebra::rows(fiberTensor) << endl;
  cout << FunG::LinearAlgebra::cols(fiberTensor) << endl;
  cout << FunG::LinearAlgebra::rows(I) << endl;
  cout << FunG::LinearAlgebra::cols(I) << endl;

  auto incompressibleNeoHooke = FunG::incompressibleNeoHooke<M,dim>(c0,I);
  auto incompressibleModifiedNeoHooke = FunG::modifiedIncompressibleNeoHooke<M,dim>(c0,I);
  auto compressibleNeoHooke = FunG::compressibleNeoHooke<Pow<2>,LN,M,dim>(c0,d0,d1,I);
  auto compressibleModifiedNeoHooke = FunG::modifiedCompressibleNeoHooke<Pow<2>,LN,M,dim>(c0,d0,d1,I);

  auto incompressibleMooneyRivlin = FunG::incompressibleMooneyRivlin<M,dim>(c0,c1,I);
  auto compressibleMooneyRivlin = FunG::compressibleMooneyRivlin<Pow<2>,LN,M,dim>(c0,c1,d0,d1,I);

  auto incompressibleSkin = FunG::incompressibleSkin_Hendriks<M,dim>(I);
  auto compressibleSkin = FunG::compressibleSkin_Hendriks<Pow<2>,LN,M,dim>(d0,d1,I);

  auto incompressibleAdipose = FunG::incompressibleAdiposeTissue_SommerHolzapfel<M,dim>(fiberTensor,I);
  auto compressibleAdipose = FunG::compressibleAdiposeTissue_SommerHolzapfel<Pow<2>,LN,M,dim>(d0,d1,fiberTensor,I);

  auto incompressibleMuscle = FunG::incompressibleMuscleTissue_Martins<M,dim>(fiberTensor,I);
  auto compressibleMuscle = FunG::compressibleMuscleTissue_Martins<Pow<2>,LN,M,dim>(d0,d1,fiberTensor,I);
...
#endif /* FUNG_PERFORMANCE_TEST_EXAMPLE_TEST_DYNAMIC_ARMADILLO_HH */
