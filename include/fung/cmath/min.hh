...
namespace FunG
{
    ...
    struct Min
    {
        ...
        auto d1( Arg&& dx ) const
        {
            ...
            return ComputeConditional< D1F, D1G >(D1F(f_, dx), D1G(g_, dx), f_smaller_than_g_)();
        }
        ...
        auto d2( ArgX&& dx, ArgY&& dy ) const
        {
            ...
            return ComputeConditional< D2F, D2G >(D2F(f_, dx, dy), D2G(g_, dx, dy), f_smaller_than_g_)();
        }
        ...
        auto d3( ArgX&& dx, ArgY&& dy, ArgZ&& dz ) const
        {
            ...
            return ComputeConditional< D3F, D3G >(D3F(f_, dx, dy, dz), D3G(g_, dx, dy, dz), f_smaller_than_g_)();
        }
    ...
    private:
        ...
        F f_;
        G g_;
        std::decay_t<decltype (std::min(std::declval<F>()(), std::declval<G>()()))> value_;
        bool f_smaller_than_g_;
    };


    template <class F, class G,
              std::enable_if_t< Checks::isFunction< std::decay_t<F> >() && Checks::isFunction< std::decay_t<G> >() >* = nullptr>
    decltype(auto) min(F&& f, G&& g)
    {
        return Min<std::decay_t<F>, std::decay_t<G>>(std::forward<F>(f), std::forward<G>(g));
    }
    ...
    decltype(auto) min(F&& f, G&& g)
    {
        return Min<std::decay_t<F>, Constant<std::decay_t<G>>>(std::forward<F>(f), constant(std::forward<G>(g)));
    }
    ...
    decltype(auto) min(F&& f, G&& g)
    {
        return Min<Constant<std::decay_t<F>>, std::decay_t<G>>(constant(std::forward<F>(f)), std::forward<G>(g));
    }
    /** @} */
}
